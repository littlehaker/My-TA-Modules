...
##
# Adeptsense format for RDoc.
# This class is used by RDoc to create an adeptsense for Ruby with a fake ctags
# file and an api file.
#
# Requires Ruby 1.9.
#
...
# `rdoc/rdoc.rb`.
#
# Usage: rdoc -f adeptsense [files]
# Two files are generated in the output directory: `tags` and `api`.
class RDoc::Generator::ADEPTSENSE
  CTAGS_FMT = "%s\t_\t0;\"\t%s\t%s\n"
  ...
  def process_context(context)
    ...
    context.each_method do |m|
      next if m.name !~ /^\w/
      write_tag(@tags, m.name, 'f', 'class:' + context.full_name)
  ...
  # @param k The kind of ctag. Ruby adeptsense uses 5 kinds: m Module, c Class,
